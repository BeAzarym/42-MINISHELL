...
const char	*read_and_append(char *already_read, int fd, ssize_t g_stat.status);
...
const char	*read_and_append(char *already_read, int fd, ssize_t g_stat.status)
{
	...
	*buff = '\0';
	while ((g_stat.status > 0) && (!ft_strchr(buff, '\n')))
	{
		g_stat.status = read(fd, buff, BUFFER_SIZE);
		if (g_stat.status < 0)
		{
			free(buff);
			free(already_read);
